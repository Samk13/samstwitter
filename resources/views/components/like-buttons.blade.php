<div class="{{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }} flex space-x-4 ml-2 ">
    ...
    <form method="POST" action="/tweets/{{ $tweet->id }}/like">
        @csrf
        <button class="flex items-center space-x-2 mt-4 ">
    ...
    <form method="POST" action="/tweets/{{ $tweet->id }}/like">
        @csrf
        @method('DELETE')
        <button class="flex items-center space-x-2 mt-4 {{ $tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }}">
